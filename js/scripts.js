...
( function( $ ) {
	...
	*/
	$('header .top-menu, .typed-bread, .popup-box .bts, .animate-to-page, .bts, .logo').on('click', 'a', function(){
	...
	$('#cform').validate({
		...
		submitHandler: function() {
			$.ajax({
				...
				type: 'post',
				dataType: 'json',
				data: 'name='+ $("#cform").find('input[name="name"]').val() + '&email='+ $("#cform").find('input[name="email"]').val() + '&message=' + $("#cform").find('textarea[name="message"]').val() + '&source='+ $("#cform").find('input[name="source"]').val() + '&medium='+ $("#cform").find('input[name="medium"]').val() + '&campaign='+ $("#cform").find('input[name="campaign"]').val() + '&content='+ $("#cform").find('input[name="content"]').val() + '&term='+ $("#cform").find('input[name="term"]').val(),
				beforeSend: function() {
				...
				success: function(data) {
					var url = "https://blockchain.evacodes.com/success" + (window.location.search || '');
...
*/
function initMap() {
	var myLatlng = new google.maps.LatLng(50.45947737507794, 30.44691697099096);
	var styles = [
		{
			"stylers": [
				{
